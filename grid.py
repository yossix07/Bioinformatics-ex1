...
class Grid:
    ...
    def init_slow_spread(self, s1_number, s2_number, s3_number, s4_number):
        ...
        for col in range(border_limit):
            ...
            top_left_edge = [self.matrix[row][col] for row in range(border_limit, border_limit + 1) if self.matrix[row][col]] + \
                                [self.matrix[row][col+1] for row in range(border_limit) if self.matrix[row][col+1]]

            bottom_left_corner = [self.matrix[row][limit-col] for row in range(border_limit) if self.matrix[row][limit-col]]
            bottom_left_edge = [self.matrix[row][limit-col] for row in range(border_limit, border_limit + 1) if self.matrix[row][limit-col]] + \
                                   [self.matrix[row][limit-col - 1] for row in range(border_limit) if self.matrix[row][limit-col - 1]]
            
            bottom_right_corner = [self.matrix[limit-row][limit-col] for row in range(border_limit) if self.matrix[limit-row][limit-col]]
            bottom_right_edge = [self.matrix[limit-row][limit-col] for row in range(border_limit, border_limit + 1) if self.matrix[limit-row][limit-col]] + \
                                [self.matrix[limit-row][limit-col-1] for row in range(border_limit) if self.matrix[limit-row][limit-col-1]]

            top_right_corner = [self.matrix[limit-row][col] for row in range(border_limit) if self.matrix[limit-row][col]]
            top_right_edge = [self.matrix[limit-row][col] for row in range(border_limit, border_limit + 1) if self.matrix[limit-row][col]] + \
                             [self.matrix[limit-row][col+1] for row in range(border_limit) if self.matrix[limit-row][col+1]]
